...
## A quick reminder on how to make ~~a Promise~~ an Honour

<CodeSurfer>

```js title="We want to be able to use it like this"
const myHonour = Honour((resolve, reject) => {
  setTimeout(() => {
    resolve('Hello, world!')
  }, 1000)
})
...
```js 1[18:24],5[2] title="Honour needs to take an argument!"
const myHonour = Honour((resolve, reject) => {
  setTimeout(() => {
    resolve('Hello, world!')
  }, 1000)
...
```js 1[25:46],2:4,5[1] title="Honour needs to take an argument!"
const myHonour = Honour((resolve, reject) => {
  setTimeout(() => {
    resolve('Hello, world!')
  }, 1000)
...
```js 1[25:40] title="Honour needs to take an argument!"
const myHonour = Honour(aHandlerFunction)
```
...
- Oh and we should only invoke it when the Honour is fulfilled
